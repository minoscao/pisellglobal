...
## Marketing Library and bounded supporting columns — 13 September

Added shared Assets / Materials records, category / language / format / status filters, card/list views, source relationships in both directions, original downloads, full previews and small WebP thumbnails. New and edited records reuse the workspace drawer and form controls. Long drawers now scroll correctly, including the material source selector and source notes. Overview uses actual library previews and module icons; its first column is bounded to 190px and target results to 310px. Global Alerts uses a primary flexible map and a companion list capped at 380px, with approximately 155px signal rows. Its Google map reuses the existing Pisell map palette.

Applied project-preserving ui-ux-pro-max guidance: compact density, optimized image thumbnails, original aspect ratios, meaningful icons, textual status beside colour, responsive/touch controls and complete upload/loading/error feedback. No fabricated assets, percentages, customer interest or commercial values.

Checks: 8 Node + 12 SQLite tests passed. Material-to-asset direction, missing references, stable type and stale-save rollback are enforced. Local HTTP checks rejected stale edits (409), invalid source links/type changes (400), and incomplete multipart completion (400); completion retry returned the same file. Production file routes reject anonymous reads (401). Original PNG download size and SHA-256 match the local source exactly. Six production thumbnails return 200; all six source links have matching inverse relationships. Category filter, six-row list, long drawer scroll, and 390px no-overflow layout passed; desktop layouts checked at 1440/1920px and list-only at 1024px.

Production contains 6 source assets and 1 v07 PNG banner material, with 6 source links. There are **zero PSD/PSB files** in the uploaded file collection; Photoshop originals stay local as requested. The banner remains Draft. Library file records total 34,164,768 bytes including full previews/thumbnails; this is not a whole-account usage or billing measurement. Production still has 12 customer orders, zero provided contract amounts and no recorded receipts. Financial test data and multipart test uploads remain local only.
