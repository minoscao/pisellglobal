...
for(const v of venues.venues){const sourceIds=[];for(const s of v.sources||[]){if(!s.url)continue;let id=sourceByUrl.get(s.url);if(!id){id='vs-'+createHash('sha256').update(s.url).digest('hex').slice(0,24);sourceByUrl.set(s.url,id);insert('sources',{id,url:s.url,publisher:s.publisher||s.name||'',payload:JSON.stringify({...s,id})});}sourceIds.push(id);}sql.push(...venueStatements(v,sourceIds));}
